...
- Keep the project on Node.js v24+, ESM, and the existing TypeScript import style, including `.ts` extensions in source imports where used.
- Do not hand-edit `dist/`; build output is generated by `pnpm build`.
- Follow the existing ESLint and neostandard setup when changing code.
- Treat repolyze as a git-history analysis tool, not a general-purpose static analyzer.
